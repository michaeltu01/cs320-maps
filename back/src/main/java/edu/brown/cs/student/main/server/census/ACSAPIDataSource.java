...
/**
 * Class which reaches out to the ACSAPI in order to retrieve state codes, county codes,
 * and ultimately, broadband percentages.
 */
public class ACSAPIDataSource implements CensusDataSource {
  ...
  private Map<String, String> storeStateCodes() throws DatasourceException {
    try {
      ...
      for (int i = 1; i < body.size(); i++) {
        ...
        stateCodeMap.put(row.get(0).toUpperCase(), row.get(1)); // index 0 = state name; index 1 = state code
      }
      return stateCodeMap;
    } catch(IOException e) {
   ...
   * @param countyName name of the county to search formatted in the format of the API county name entries
   * @return the county code as a String
   * @throws DatasourceException if issue occurs with requesting data from data source
   */
  private String getCountyCode(String stateCode, String countyName) throws DatasourceException, BadJsonException {
    try {
  ...
  /**
   * This method retrieves the percentage of households with broadband access from the ACS API
   * for a target location given by the target state and county.
   *
