...
public class LoadCSVTests {
  ...
  public void loadCSVTestsWithHeaders() throws IOException, DatasourceException {
    ...
    HttpURLConnection clientConnection =
        tryRequest("loadcsv?filename=data/test/simple-for-tests&headers=true");
  ...
  public void testLoadBadCSV() throws IOException {
    HttpURLConnection clientConnection =
        tryRequest("loadcsv?filename=simple-for-tests&headers=false");
    assertEquals(200, clientConnection.getResponseCode());
    ...
    //    assertEquals("File path not found:
    // C:\\Code\\CS320\\server-louietanmay-michaeltu01\\data\\simple-for-tests",
    // body.get("details"));
    assertEquals("File path not found: simple-for-tests", body.get("details"));
  }

  /**
   * Makes sure that loadCSV throws the appropriate error when a filename is not provided in the
   * query.
   *
   * @throws IOException
   * @throws DatasourceException
  ...
  public void loadMoreThanOneCSV() throws IOException, DatasourceException {
    ...
    HttpURLConnection clientConnectionBefore =
        tryRequest("loadcsv?filename=data/test/simple-for-tests&headers=true");
    assertEquals(200, clientConnectionBefore.getResponseCode());

    HttpURLConnection clientConnection =
        tryRequest("loadcsv?filename=data/test/simple-for-tests-2&headers=true");
    assertEquals(200, clientConnection.getResponseCode());
